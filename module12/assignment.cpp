...
const int SUB_BUFFER_DIM_X = 2;
const int SUB_BUFFER_DIM_Y = 2;
const int SUB_BUFFER_SIZE = SUB_BUFFER_DIM_X * SUB_BUFFER_DIM_Y;
...
int main(int argc, char** argv)
{
    ...
    std::cout << "NUM DEVICES: " << numDevices << "\n";
    ...
    const int numSubBuffers = NUM_BUFFER_ELEMENTS / ( SUB_BUFFER_SIZE );

    for ( unsigned int i = 0; i < numSubBuffers; i++ )
    {
        cl_buffer_region region = 
        { 
            SUB_BUFFER_SIZE * i * sizeof(float),
            SUB_BUFFER_SIZE * sizeof(float), 
        };
    ...
    std::cout << "SUB BUFFER COUNT: " << buffers.size() - 1 << "\n";
    ...
    for (unsigned int i = 0; i < queues.size(); i++)
    {
        ...
        size_t gWI = SUB_BUFFER_SIZE;//NUM_BUFFER_ELEMENTS;
    ...
    std::cout << "Program completed successfully, average kernel took " << get_duration_ns( start, stop ) << " ns.\n";
